...
public interface ProductRepository extends JpaRepository<Product, Integer> {
	...
	@Query("SELECT product FROM Product product WHERE "
			...
			+ "(?9 is null OR title like ?9 OR brand like ?9 OR manufacturer like ?8)")
	Page<Product> search(String title, String brand, Date publishFrom, Date publishUntil, String manufacturer,
	...
	@Query("SELECT product FROM Product product WHERE "
			...
			+ "(?10 is null OR title like ?10 OR brand like ?10 OR manufacturer like ?10)")
	Page<Product> search(String title, String brand, List<Category> categories, Date publishFrom, Date publishUntil, String manufacturer,
			Double priceFrom, Double priceTo, boolean hideUnavailable, String query, Pageable pageable);
}