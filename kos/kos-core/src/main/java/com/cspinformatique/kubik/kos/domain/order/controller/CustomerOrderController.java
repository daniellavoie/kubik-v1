...
public class CustomerOrderController {
	...
	public Page<CustomerOrder> findByAccount(Pageable pageable, Principal principal) {
		return customerOrderService.findByAccountAndStatusIn(accountService.findByPrincipal(principal), HISTORY_STATUS,
				pageable);
	}
	...
	@RequestMapping(value = "/{id}/invoice", produces = MediaType.APPLICATION_PDF_VALUE)
	public @ResponseBody ResponseEntity<InputStreamResource> loadInvoicePdf(@PathVariable long id) {
		return customerOrderService.loadInvoicePdf(id)
				.map(inputStream -> new ResponseEntity<>(new InputStreamResource(inputStream), HttpStatus.OK))
