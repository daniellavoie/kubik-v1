(function(){
	...
	function PurchaseOrderDetailsCtrl(productService, purchaseOrderService, $scope, $http, $timeout, $controller){
		var vm = this;
		...
		function loadOrder(){
			...
			function findOneSuccess(purchaseOrder){
				vm.order = purchaseOrder;
		...
		function searchProduct(){
			...
			if(vm.typedEan13 != "" && !vm.searchInProgress){
				...
				productService
					...
					.then(handleResults)
					.finally(searchCompleted);

				vm.search.ean13 = "";
			}
			
			function handleResults(product){
