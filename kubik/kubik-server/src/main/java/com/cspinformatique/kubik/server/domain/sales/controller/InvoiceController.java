...
public class InvoiceController {
	...
	public void generateReceiptPdf(@PathVariable int invoiceId, ServletResponse response) {
		try {
			JasperExportManager.exportReportToPdfStream(
					this.reportService.generateReceiptReport(invoiceService.findOne(invoiceId)),
	...
	public void generateReceiptPrintJob(@PathVariable int invoiceId) {
		this.printService.createReceiptPrintJob(this.invoiceService.findOne(invoiceId));
	}

	@RequestMapping(method = RequestMethod.GET, produces = MediaType.APPLICATION_JSON_VALUE)
	public @ResponseBody Page<Invoice> find(@RequestParam(required = false) String status,
		...
		PageRequest pageRequest = new PageRequest(page, resultPerPage, direction != null ? direction : Direction.DESC,
				sortBy);

		if (status != null) {
			return this.invoiceService.findByStatus(invoiceStatusService.findOne(status), pageRequest);
		} else {
	...
	public @ResponseBody InvoiceDetail findDetailByInvoiceIdAndProductEan13(@PathVariable int id,
			@PathVariable String ean13) {
		return this.invoiceService.findDetailByInvoiceIdAndProductEan13(id, ean13);
	}
	...
	@RequestMapping(value = "/{invoiceId}/payment", method = { RequestMethod.POST,
			RequestMethod.PUT }, produces = MediaType.APPLICATION_JSON_VALUE)
	public @ResponseBody Iterable<Payment> savePayments(Iterable<Payment> payments) {
