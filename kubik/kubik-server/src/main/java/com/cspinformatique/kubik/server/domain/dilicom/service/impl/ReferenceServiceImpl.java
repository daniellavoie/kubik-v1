...
public class ReferenceServiceImpl implements ReferenceService {
	...
	public Reference buildReferenceFromProduct(Product product) {
		...
		return new Reference(oldReference.getId(), product.getEan13(), product.getSupplier().getEan13(),
				oldReference.getPriceApplicationOrAvailabilityDate(), oldReference.getAvailability(),
				oldReference.getPriceType(), product.getPriceTaxIn(), product.getSchoolbook(), product.getTvaRate1(),
				product.getPriceTaxOut1(), product.getTvaRate2(), product.getPriceTaxOut2(), product.getTvaRate3(),
				product.getPriceTaxOut3(), product.getReturnType() != null ? product.getReturnType().getCode() : null,
				product.getAvailableForOrder(), product.getDatePublished(),
				product.getProductType() != null ? product.getProductType().getCode() : null,
				product.getPublishEndDate(), product.getStandardLabel(), product.getCashRegisterLabel(),
				product.getThickness(), product.getWidth(), product.getHeight(), product.getWeight(),
				product.getExtendedLabel(), product.getPublisher(), product.getCollection(), product.getAuthor(),
				product.getPublisherPresentation(), product.getIsbn(), product.getSupplierReference(),
				product.getCollectionReference(), product.getTheme(), product.getPublisherIsnb(),
				product.getReplacesAReference(), product.getReplacedByAReference(), product.getReplacesEan13(),
				product.getReplacedByEan13(), product.getOrderableByUnit(),
				product.getBarcodeType() != null ? product.getBarcodeType().getCode() : null,
				product.getMainReference(), product.getSecondaryReference(), product.getReferencesCount(), true,
				product.getImageEncryptedKey(), new Date(), new Date());
	}

	private List<Reference> calculateImageUrl(List<Reference> references) {
	...
	private Reference calculateImageUrl(Reference reference) {
		reference.setImageEncryptedKey(
				this.dilicomImageService.getEncryptedUrl(reference.getEan13(), reference.getSupplierEan13()));
	...
	public Iterable<Reference> cleanDoubles(Iterable<Reference> references) {
		...
		for (Reference reference : references) {
			...
			if (!referencesMap.containsKey(key)) {
				...
				LOGGER.warn("Cleaning reference double for ean13 " + reference.getEan13() + " from supplier "
						+ reference.getSupplierEan13() + ". Reference id " + reference.getId() + ".");
	...
	public void delete(String ean13, String supplierEan13) {
		this.referenceRepository.delete(this.findByEan13AndSupplierEan13(ean13, supplierEan13).getId());
	}

	@Override
	public Reference find(String ean13, String supplierEan13, boolean importedInKubik) {
		return this.referenceRepository
				.findByEan13AndSupplierEan13AndImportedInKubik(ean13, supplierEan13, importedInKubik).orElse(null);
	}

	@Override
	public List<Reference> findByEan13(String ean13) {
		return this.calculateImageUrl(this.referenceRepository.findByEan13(ean13));
	}

	@Override
	public List<Reference> findByEan13AndImportedInKubik(String ean13, boolean importedInKubik) {
		return this.referenceRepository.findByEan13AndImportedInKubik(ean13, importedInKubik);
	}

	@Override
	public Reference findByEan13AndSupplierEan13(String ean13, String supplierEan13) {
		...
		for (Reference referenceToClean : references) {
			if (!firstResult) {
				LOGGER.warn("Deleting double reference " + reference.getEan13() + " from supplier "
						+ reference.getSupplierEan13() + " created at " + reference.getCreationDate() + ".");

				this.referenceRepository.delete(referenceToClean.getId());
			}
	...
	@Override
	public Page<Reference> findByImportedInKubik(boolean importedInubik, Pageable pageable) {
		return this.calculateImageUrl(this.referenceRepository.findByImportedInKubik(importedInubik, pageable));
	}
	...
	public Reference save(Reference reference) {
		referenceRepository.save(Arrays.asList(new Reference[] { reference }));

		return calculateImageUrl(reference);
	}
	...
	public void save(Iterable<Reference> references) {
		...
		for (Reference reference : references) {
			try {
				...
				referenceMap.put(reference.getEan13() + "-" + reference.getSupplierEan13(), reference);
			} catch (RuntimeException ex) {
				LOGGER.error("Error while processing reference " + reference.getEan13() + " from supplier "
						+ reference.getSupplierEan13(), ex);
			}
		}

		referenceRepository.save(new ArrayList<>(referenceMap.values()));
